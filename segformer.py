...
def _otsu_threshold_from_prob_map(prob_map: np.ndarray) -> float:
    """Compute Otsu threshold for a probability map in [0, 1]."""
...
def _binarize_with_otsu(outputs: torch.Tensor):
    """Apply per-image Otsu thresholding to model outputs (N,1,H,W)."""
...
def save_predictions(model, loader, device, num_examples=5, results_dir="./results"):
    ...
    with torch.no_grad():
        ...
        preds, otsu_thresholds = _binarize_with_otsu(outputs)

    print(f"Threshold Otsu médio do batch: {np.mean(otsu_thresholds):.4f}")
