...
class CBERS4HydroGuidedDataset(Dataset):
    """
    Usa CBERS4MUXDatasetWHidrography para manter transformações alinhadas,
    ...
    def __getitem__(self, idx):
        ...
        h_mid = image_wo_hydro.shape[1] // 2
        w_mid = image_wo_hydro.shape[2] // 2
        if crop_idx == 0:  # top-left
            h_slice, w_slice = slice(0, h_mid), slice(0, w_mid)
        elif crop_idx == 1:  # top-right
            h_slice, w_slice = slice(0, h_mid), slice(w_mid, image_wo_hydro.shape[2])
        elif crop_idx == 2:  # bottom-left
            h_slice, w_slice = slice(h_mid, image_wo_hydro.shape[1]), slice(0, w_mid)
        else:  # bottom-right
            h_slice, w_slice = slice(h_mid, image_wo_hydro.shape[1]), slice(w_mid, image_wo_hydro.shape[2])

        image_wo_hydro = image_wo_hydro[:, h_slice, w_slice]
...
def save_predictions(model, loader, device, num_examples=5, results_dir="./results"):
    print("Salvando exemplos de previsoes com fusao guiada por hidrografia...")
    model.eval()
    ...
    with torch.no_grad():
        ...
        preds = _guided_binarize(outputs, hydro)
    ...
    for i in range(min(num_examples, len(images_cpu))):
        ...
        axes[3].set_title("Predicao Fused")
        axes[3].axis("off")
...
def execute_deeplab_train(model, device, train_loader, val_loader, loss_fn, num_epochs: int, results_dir="./results"):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for images, masks, hydro in vbar:
                ...
                preds_fused = _guided_binarize(outputs, hydro)
                ...
                vbar.set_postfix({"val_loss": f"{loss.item():.4f}", "fused_iou": f"{iou_fused.item():.4f}"})
        ...
        if avg_val_iou_fused > best_fused_iou:
            best_fused_iou = avg_val_iou_fused
            best_epoch = epoch + 1
            os.makedirs(results_dir, exist_ok=True)
            model_path = os.path.join(results_dir, "best_model.pth")
            torch.save(model.state_dict(), model_path)
            print(f"Novo melhor modelo salvo em {model_path} com Fused IoU: {best_fused_iou:.4f}")
    ...
    print(f"\nTreinamento concluido. Melhor Fused IoU: {best_fused_iou:.4f}")
...
for train_num in range(1):
    for train_name, config in trains.items():
        for backbone in backbones:
            print(f"\n{'=' * 80}\nExperimento: {train_name} | Backbone: {backbone} [DeepLabV3 Hydro-Guided]\n{'=' * 80}")
            ...
            # Entram apenas 7 canais: [B, G, R, NIR, NDVI, NDWI, GNDVI]
            ...
            results_dir = f"./experiments/stats_dl/{train_num}/results_{train_name}_{backbone}_deeplabv3_hydro_guided"
            ...
            summary_line = (
                ...
                f"  -> Melhor Fused IoU de Validacao: {final_metrics['best_fused_iou']:.4f} "
...
summary_filepath = "final_results_summary_hydro_guided.txt"
print(f"\nSalvando resumo final em: {summary_filepath}")

with open(summary_filepath, "w", encoding="utf-8") as f:
    f.write("--- RESUMO FINAL DOS EXPERIMENTOS (DEEPLABV3 HYDRO-GUIDED) ---\n")
