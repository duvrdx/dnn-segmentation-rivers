...
from siamese_cross_attention import (
    SiameseCrossAttentionNet,
...
def train_simple(
    ...
    num_epochs, results_dir, backbone, hydro_weight=0.3,
):
    model.to(device)
    ...
    print(f"\nConcluido. Melhor IoU: {best_iou:.4f} (epoca {best_epoch}), WIoU: {best_wiou:.4f}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Teste rapido Siamese CrossAttention")
    parser.add_argument("--data_dir", type=str, default="../datasets/doce_256",
                        help="Diretorio do dataset")
    parser.add_argument("--backbone", type=str, default="timm-efficientnet-b7",
                        choices=["timm-efficientnet-b7", "mit_b5", "resnet152"])
    ...
    print(f"Treino: {len(train_idx)} | Val: {len(val_idx)}")

    train_loader, val_loader = create_dataloaders(
        train_paths, val_paths, img_size=128, batch_size=args.batch_size
    )

    model = SiameseCrossAttentionNet(encoder_name=args.backbone)
    ...
    print(f"  Melhor WIoU: {metrics['best_wiou']:.4f}")
