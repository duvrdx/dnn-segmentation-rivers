...
class CBERS4HydroGuidedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        _, h, w = image_wo_hydro.shape
        h2, w2 = h // 2, w // 2

        if crop_idx == 0:  # superior esquerda
            sl_h, sl_w = slice(0, h2), slice(0, w2)
        elif crop_idx == 1:  # superior direita
            sl_h, sl_w = slice(0, h2), slice(w2, w)
        elif crop_idx == 2:  # inferior esquerda
            sl_h, sl_w = slice(h2, h), slice(0, w2)
        else:  # inferior direita
            sl_h, sl_w = slice(h2, h), slice(w2, w)

        image_crop = image_wo_hydro[:, sl_h, sl_w]
        mask_crop = mask[:, sl_h, sl_w]
        hydro_crop = hydro[:, sl_h, sl_w]

        return image_crop, mask_crop, hydro_crop
    ...
    for train_name, config in trains.items():
        for backbone in backbones:
            print(
                ...
                f"[DeepLabV3 Hydro+NDWI Guided Crop4]\n{'=' * 80}"
            )
            ...
            results_dir = (
                ...
                f"results_{train_name}_{backbone}_deeplabv3_hydro_ndwi_guided_crop4"
            )
...
summary_filepath = "final_results_summary_hydro_ndwi_guided_crop4.txt"
