...
def create_dataloaders(
    ...
    train_loader = DataLoader(train_loader_dataset, batch_size=batch_size,
                              shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_loader_dataset, batch_size=batch_size,
...
def execute_unet_train(model, device: torch.device, train_loader: DataLoader,
        ...
        with torch.no_grad():
            ...
            for images, masks in vbar:
                ...
                tp, fp, fn, tn = smp.metrics.get_stats(
                    outputs, masks.long(), mode='binary', threshold=0.5)
...
for train_num in range(1):
    for train_name, config in trains.items():
        for backbone in backbones:
            ...
            """
            TODO: Mudar os modelos de treinamento para Unet++ e comparar os resultados
            ...
            """
            model = smp.Unet(
                encoder_name=backbone, encoder_weights="imagenet", in_channels=7, classes=1, activation='sigmoid')
            device = torch.device(
            ...
            final_metrics = execute_unet_train(
                ...
                loss_fn=combined_loss, num_epochs=5, results_dir=results_dir
            )
